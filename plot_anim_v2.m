function r = plot_anim_v2(x,est, measr, obj_idx_occlude, all_particles, global_vars,  eff_sample_size, relative_probs, models_to_consider)
...
%subplot(3,9,10);
%set(gca,'Position',[0.02 0.25 0.95 0.35]);
width_shift= 0.95/9;
eps = 0.04;
...
for ii = 2:size(x_pos,2)
    clf;
    % draw the relative probs via bayes factor=====
    
% % %       subplot(3,9,21);
% % %     set(gca,'Position',[0.7 0.01 0.25 0.15]);
% % %     plot(relative_probs(3,1:ii),'.-');
% % %     ax= gca;
% % %     ax.XLim = [1,size(relative_probs,2)];
% % %     ax.YLim =[-.02,1.2];
% % %     title(strcat('Prob. of #cars =', num2str(models_to_consider(3))), 'FontSize', 10);
% % %     
% % %     
    subplot(3,9,20);
    set(gca,'Position',[0.35 0.01 0.25 0.15]);
    plot(relative_probs(2,1:ii),'.-');
    ax= gca;
    ax.XLim = [1,size(relative_probs,2)];
    ax.YLim =[-.02,1.2];
    title(strcat('Prob. of #cars =', num2str(models_to_consider(2))), 'FontSize', 10);

    
    
    
    subplot(3,9,19);
    set(gca,'Position',[0.03 0.01 0.25 0.15]);
    plot(relative_probs(1,1:ii),'.-');
    ax= gca;
    ax.XLim = [1,size(relative_probs,2)];
    ax.YLim =[-.02,1.2];
    title(strcat('Prob. of #cars =', num2str(models_to_consider(1))), 'FontSize', 10);
    ...
    for iter = 1: size(global_vars.occlusion_blocs_cord,1)
        a = histogram(global_vars.occlusion_blocs_cord(iter,:),[0,global_vars.occlusion_blocs_cord(iter,1), global_vars.occlusion_blocs_cord(iter,2)+1, x(1,end)], 'EdgeColor', 'none', 'FaceColor','b');
...
% %         text(global_vars.occlusion_blocs_cord(iter,2),0.5,tmp, 'VerticalAlignment','top','HorizontalAlignment', 'right','FontSize',10);
    ...
    subplot(3,9,1);
    %clf;
    set(gca,'Position',[0.05 0.67 width_shift-eps 0.30])
    ...
    plot(repmat(measr(1,ii),2),[0,max(h.Values)],'r-', 'LineWidth',2);
    %plot([0,0],[0,max(h.Values)],'r-', 'LineWidth',2);
    ...
    hold on;
    set(gca,'Position',[0.05 + width_shift 0.67 width_shift-eps 0.3])
...
%     ax.XLim = [0, global_vars.v_max];
    ...
    hold on;
    set(gca,'Position',[0.05+ 2*width_shift 0.67 width_shift-eps 0.3])
    ...
    hold on;
    set(gca,'Position',[0.05+ 3*width_shift 0.67 width_shift-eps 0.3])
    ...
    hold on;
    set(gca,'Position',[0.05+ 4*width_shift 0.67 width_shift-eps 0.3])
    ...
    hold on;
    set(gca,'Position',[0.05+ 5*width_shift 0.67 width_shift-eps 0.3])
    ...
    hold on;
    set(gca,'Position',[0.05+ 6*width_shift 0.67 width_shift-eps 0.3])
    ...
    hold on;
    set(gca,'Position',[0.05+ 8*width_shift 0.67 width_shift-eps 0.3])
    ...
    hold on;
    set(gca,'Position',[0.05+ 7*width_shift 0.67 width_shift-eps 0.3])
