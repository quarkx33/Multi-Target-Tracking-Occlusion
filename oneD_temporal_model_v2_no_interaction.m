...
function states = oneD_temporal_model_v2_no_interaction(init_state, temporal_params, T, num_objects, global_vars)
...
for ii = 2: T
    ...
    for jj = 1: num_objects
        ...
        states(3*jj,ii) = min(max(states(3*jj,ii), -states((jj-1)*3+2,ii)/global_vars.delta_t), (global_vars.v_max - states((jj-1)*3+2,ii))/global_vars.delta_t);
