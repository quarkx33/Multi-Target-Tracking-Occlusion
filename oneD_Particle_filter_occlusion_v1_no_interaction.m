%% Particle filter implementation (NO interaction case)

function [posterior_states, posterior_cov, num_failures, posterior_states1, posterior_cov1, posterior_states_resamp, posterior_cov_resamp, all_particles, eff_sample_size ] = oneD_Particle_filter_occlusion_v1_no_interaction(init_state, obj_idx_occlude, measurements, temporal_params, measurement_params, num_particles, global_vars, states)
...
% % % %         tmp = oneD_temporal_model_v2_no_interaction(particles(:,jj), temporal_params, 2, num_objects, global_vars);
...
% % % %     
    ...
    particles_new = oneD_temporal_model_v2_no_interaction_vectorised(particles, temporal_params, num_objects, global_vars);
...
% % % %             particles_new(1:3:end,:) = tmp1;
            ...
            for jj = 1: num_particles
                tmp = oneD_temporal_model_v2_no_interaction(states(:,t-1), temporal_params, 2, num_objects, global_vars);
                particles_new(:,jj) = tmp(:,2);
