...
%     end
    ...
    states = oneD_temporal_model_v2_no_interaction(init_state, temporal_params, trials, num_objects, global_vars);
    
    
   %Set the occlusion positions
    tmp = states(1:3:end,:);
    for iter = 1 : size(global_vars.occlusion_blocs_cord,1)
        obj_idx_occlude = obj_idx_occlude & ((tmp < global_vars.occlusion_blocs_cord(iter,1)) | (tmp > global_vars.occlusion_blocs_cord(iter,2)));
...
%     MSE_kalman(ii,:) = sum(tmp .^2,1); 
   ...
   % [wts, particles] = oneD_Particle_filter_v1(measurements, temporal_params, measurement_params, num_particles, global_vars);
     ...
     [posterior_states_mean, posterior_states_cov, num_fail, posterior_states_mean_preupdate, posterior_states_cov_preupdate, posterior_states_resamp, posterior_cov_resamp, all_particles, eff_sample_size] = oneD_Particle_filter_occlusion_v1_no_interaction(init_state, logical(obj_idx_occlude), measurements, temporal_params, measurement_params, num_particles, global_vars, states);
